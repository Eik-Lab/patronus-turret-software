  ...
  for (l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
    ...
    for (l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      if (obj_meta->class_id == PGIE_CLASS_ID_DRONE) {
        if (g_output_rgb) {
          patronus::core::Detection d;
          d.left_ = obj_meta->rect_params.left;
          d.top_ = obj_meta->rect_params.top;
          d.width_ = obj_meta->rect_params.width;
          d.height_ = obj_meta->rect_params.height;
          d.class_id_ = obj_meta->class_id;
          d.confidence_ = obj_meta->confidence;
          g_output_rgb->push(d);
        }
        drone_count++;
      }
...
int run_pipeline_rgb(const patronus::config::PipelineConfig &config,
  ...
  char *infer_config_abs = realpath(config.infer_config_.c_str(), NULL);

  yaml_config = infer_config_abs && (g_str_has_suffix(infer_config_abs, ".yml") ||
                                     g_str_has_suffix(infer_config_abs, ".yaml"));

  if (yaml_config) {
    RETURN_ON_PARSER_ERROR(nvds_parse_gie_type(&pgie_type, infer_config_abs, "primary-gie"));
  }

  pipeline = gst_pipeline_new("patronus-rgb-pipeline");

  source = gst_element_factory_make("pylonsrc", "pylon-source");
  g_object_set(G_OBJECT(source), "device-serial-number", config.camera_serial_.c_str(), NULL);

  capsfilter_src = gst_element_factory_make("capsfilter", "caps-src");
  caps_src = gst_caps_from_string(config.camera_caps_.c_str());
  ...
  g_object_set(G_OBJECT(encoder), "bitrate", config.encoder_bitrate_, NULL);
  ...
  g_object_set(G_OBJECT(streammux), "width", config.muxer_width_, "height", config.muxer_height_,
               "live-source", TRUE, "batched-push-timeout", MUXER_BATCH_TIMEOUT_USEC, NULL);

  if (yaml_config && infer_config_abs) {
    RETURN_ON_PARSER_ERROR(nvds_parse_streammux(streammux, infer_config_abs, "streammux"));
    RETURN_ON_PARSER_ERROR(nvds_parse_gie(pgie, infer_config_abs, "primary-gie"));
  } else if (infer_config_abs) {
    g_object_set(G_OBJECT(pgie), "config-file-path", infer_config_abs, NULL);
  } else {
    g_object_set(G_OBJECT(pgie), "config-file-path", config.infer_config_.c_str(), NULL);
  }
  free(infer_config_abs);
