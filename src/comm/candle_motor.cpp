...
CandleMotor::CandleMotor(const std::vector<GimbalCfg>& gimbals,
                         const BusCfg& bus)
...
std::vector<uint16_t> CandleMotor::try_resolve_motor_ids()
{
  ...
  if (discovered.size() < needed) {
    std::fprintf(stderr,
                 "CandleMotor: found %zu MD(s) on bus — need %zu for %zu gimbal(s)\n",
...
{
  try {
    candle_ = mab::attachCandle(to_datarate(bus_cfg_.datarate),
  ...
  for (size_t g = 0; g < gimbals_.size(); ++g) {
    pan_motors_[g]  = std::make_unique<mab::MD>(gimbals_[g].pan_node_id, candle_);
    tilt_motors_[g] = std::make_unique<mab::MD>(gimbals_[g].tilt_node_id, candle_);
  }
  ...
  for (size_t g = 0; g < gimbals_.size(); ++g) {
    ...
    std::printf("CandleMotor: gimbal %zu — pan=MD%u tilt=MD%u  tilt gear %.1f:1\n",
                g, pan_motors_[g]->m_canId, tilt_motors_[g]->m_canId,
                tilt_gear_ratios_[g]);
  }
...
{
  if (gimbal >= gimbals_.size()) return;
  auto* pan  = pan_motors_[gimbal].get();
  auto* tilt = tilt_motors_[gimbal].get();
  if (!pan || !tilt) return;

  float max_vel = gimbals_[gimbal].max_velocity_rad_s;
...
{
  if (gimbal >= gimbals_.size()) return {0, 0};
  if (!pan_motors_[gimbal] || !tilt_motors_[gimbal]) return {0, 0};
...
{
  if (gimbal >= gimbals_.size()) return false;
  auto* pan  = pan_motors_[gimbal].get();
  auto* tilt = tilt_motors_[gimbal].get();
  if (!pan || !tilt) return false;
...
bool CandleMotor::return_to_home(size_t gimbal, float position_gain,
                                 float tolerance_rad,
                                 float max_velocity_rad_s)
{
  if (gimbal >= gimbals_.size()) return true;
  auto* pan  = pan_motors_[gimbal].get();
  auto* tilt = tilt_motors_[gimbal].get();
  if (!pan || !tilt) return true;

  float limit = (max_velocity_rad_s > 0.0F) ?
      std::min(max_velocity_rad_s, gimbals_[gimbal].max_velocity_rad_s) :
      gimbals_[gimbal].max_velocity_rad_s;
...
{
  if (gimbal >= gimbals_.size()) return nullptr;
